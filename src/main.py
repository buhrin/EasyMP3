...
def download_audio(task_id, link, output_dir, status_callback, app):
    ...
    try:
        ...
        command = [
            ...
            "--add-metadata",
            "--output",
            output_template,  # Output to temp subdir
...
def crop_thumbnail(task_id, mp3_file, status_callback, app):
    ...
    try:
        ...
        if result_extract.returncode != 0:
            if (
                "error retrieving cover art" in result_extract.stderr.lower()
                or "attached picture extraction failed" in result_extract.stderr.lower()
            ):
        ...
        if not temp_image_name.exists():
            ...
            print(
                f"Thumbnail file {temp_image_name} not found after extraction attempt for {mp3_file.name}. Skipping crop."
            )
            return True  # Not an error, just no thumbnail to crop
        ...
        cmd_embed = [
            ...
            "-y",
            "-i",
            str(mp3_file),  # Input MP3
            "-i",
            str(cropped_image_name),  # Input Cropped JPG
            "-map_metadata",
            "0",
            "-map_metadata:s:1",
            "0:s:1",  # Map image metadata
            "-map",
            "0:a",  # Map audio stream
            "-map",
            "1",  # Map new image stream
            "-acodec",
            "copy",
        ...
        # Exceptions specifically for the embedding step
            ...
            error_message = (
                f"ffmpeg error during embed (Code: {e.returncode}) for {mp3_file.name}. Stderr: {e.stderr[:200]}..."
            )
...
class EasyMP3App:
    def __init__(self, root):
        ...
        self.output_dir_label = ttk.Label(
            self.controls_frame, textvariable=self.output_dir_var, relief="sunken", padding=(5, 2)
        )
        ...
        self.clipboard_button = ttk.Button(
            self.buttons_frame, text="Download from Clipboard", command=self.download_from_clipboard
        )
        ...
        try:
            if self.task_tree.exists(item_id):
                ...
                if col_index is not None:  # Check if column name was valid
                    ...
                    print(
                        f"!!! Warning: Invalid column name '{column}' passed to update_task_display for item {item_id}"
                    )
